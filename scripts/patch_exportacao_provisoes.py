...
if 'function criarXlsxBaseAnaliticaProvisoes' not in s:
    assert anchor in s
    s = s.replace(anchor, helper + anchor, 1)

fn_start = s.index('function TelaProvisoes(')
state_anchor = '  const [carregando, setCarregando] = useState(false);\n'
state_pos = s.index(state_anchor, fn_start)
if 'exportandoProvisoes' not in s[fn_start:state_pos + 500]:
    s = s[:state_pos + len(state_anchor)] + '  const [exportandoProvisoes, setExportandoProvisoes] = useState(false);\n' + s[state_pos + len(state_anchor):]

function_anchor = '  const abrirNova = () => {'
insert_pos = s.index(function_anchor, fn_start)
export_fn = r'''  const exportarBaseAnaliticaProvisoes = () => {
...
'''
if 'const exportarBaseAnaliticaProvisoes' not in s[fn_start:insert_pos]:
    s = s[:insert_pos] + export_fn + s[insert_pos:]

button_anchor = '<Btn variant="primary" onClick={abrirNova}>+ Nova provisão</Btn>'
replacement = '<div style={{ display: \'flex\', gap: \'8px\', flexWrap: \'wrap\' }}><Btn variant="secondary" onClick={exportarBaseAnaliticaProvisoes} disabled={exportandoProvisoes || provisoes.length === 0}>{exportandoProvisoes ? \'Exportando...\' : \'📊 Exportar base analítica\'}</Btn><Btn variant="primary" onClick={abrirNova}>+ Nova provisão</Btn></div>'
